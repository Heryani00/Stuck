...
@section('container')
  <form class="space-y-8 divide-gray-200" method="post" action="/dashboard/profile/{{ $user->id }}" enctype="multipart/form-data">
    @method('put')
    @csrf
    <div class="space-y-8 divide-y divide-gray-200 sm:space-y-5 ml-10">
      <div>
        <div>
        ...
        <div class="mt-6 sm:mt-5 space-y-6 sm:space-y-5">
          <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
            <label for="name" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2"> Username </label>
          ...
          <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
            ...
            <div class="mt-1 sm:mt-0 sm:col-span-2">
              ...
              <p class="mt-2 mb-2 text-sm text-gray-500">Write a few sentences about yourself.</p>
            </div>
          </div>
          </div>
...
<div class="mx-4">
  <form class="space-y-8" method="post" action="/dashboard/profile/{{ $user->id }}" enctype="multipart/form-data">
    ...
    <div class="space-y-8 divide-y divide-gray-200 sm:space-y-5"> 
      <div>
        <div>
          <h3 class="text-7xl rotate-180 leading-6 font-medium text-pink-500">Profile</h3>
          <p class="mt-1 max-w-2xl text-sm text-gray-500">This information will be displayed publicly so be careful what you share.</p>
        </div>
        ...
        <div class="mt-6 sm:mt-5 space-y-6 sm:space-y-5">
          <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
            ...
            <div class="mt-1 sm:mt-0 sm:col-span-2">
              <div class="max-w-lg flex rounded-md shadow-sm">
                <input type="text" name="name" id="name" autocomplete="name" class="flex-1 block w-full focus:ring-indigo-500 focus:border-indigo-500 min-w-0 rounded-none rounded-r-md sm:text-sm border-gray-300" value="{{ $user->name }}">
              </div>
            </div>
          </div>
        <div class="mt-6 sm:mt-5 space-y-6 sm:space-y-5">
          <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
            ...
            <div class="mt-1 sm:mt-0 sm:col-span-2">
              <div class="max-w-lg flex rounded-md shadow-sm">
                <input type="text" name="username" id="username" autocomplete="username" class="flex-1 block w-full focus:ring-indigo-500 focus:border-indigo-500 min-w-0 rounded-none rounded-r-md sm:text-sm border-gray-300" value="{{ $user->username }}">
              </div>
            </div>
          </div>
